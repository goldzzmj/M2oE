...
class NearestEmbedFunc(Function):
    ...
    def forward(ctx, input, emb):
        ...
        x_expanded = input.unsqueeze(-1) # (512,10,20,1)     # [batch_size, emb_dim, 1]
        num_arbitrary_dims = len(ctx.dims) - 2  
        if num_arbitrary_dims:
            emb_expanded = emb.view(emb.shape[0], *([1] * num_arbitrary_dims), emb.shape[1]) # (10,1,10)
        else:
            emb_expanded = emb

        # find nearest neighbors
        dist = torch.norm(x_expanded - emb_expanded, p=2, dim=1) # (512, 20, 10)
        _, argmin = dist.min(-1) # _:(512,20), argmin:(512,20)(index:int)
        shifted_shape = [input.shape[0], *list(input.shape[2:]) ,input.shape[1]] # [512,20,10]
        result = emb.t().index_select(0, argmin.view(-1)).view(shifted_shape).permute(0, ctx.dims[-1], *ctx.dims[1:-1]) # (512,10,20)
