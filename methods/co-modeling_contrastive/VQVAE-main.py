...
from vqvae.auto_encoder import *
...
# Use VQ-VAE parameter
parser.add_argument('--k', type=int, default=10, help='codebook length')
...
def main():
    ...
    train_dataset_graph, train_label = dgl.load_graphs(
        r'E:\RepCon-main\graph dataset\{}\train.bin'.format(args.dataset))
    valid_dataset_graph, valid_label = dgl.load_graphs(
        r'E:\RepCon-main\graph dataset\{}\valid.bin'.format(args.dataset))
    ...
    if args.mode == 'train':
        # 10,64,64,64
        x_size, processed_x_size, b_size, z_size, k = args.src_len, args.hidden, args.hidden, args.hidden, args.k
        # Seq VQVAE
        Seq_model = VQ_VAE(args, k=k).to(device)
        Seq_optimizer = optim.Adam(Seq_model.parameters(), lr=args.seq_lr)
        Seq_scheduler = CosineAnnealingLR(Seq_optimizer, T_max=args.epochs // 5, eta_min=1e-5)
        ...
        Graph_scheduler = CosineAnnealingLR(Graph_optimizer, T_max=args.epochs // 5, eta_min=1e-5)
        ...
        for epoch in range(args.epochs):
            ...
            for i, (graphs, sequences, labels, pad_len) in enumerate(train_loader):
                ...
                # predict:(512,1); hidden:(512,640); z_e:(512, 10, 20); emb:(512, 10, 20)
                predict, hidden, z_e, emb = Seq_model(sequences)
                if args.use_vae:
                    graph_outputs, graph_hid, mean2, log_std2 = Graph_model(graphs)
                else:
                    graph_outputs, graph_hid = Graph_model(graphs)

                loss_seq = Seq_model.loss_function(predict, labels, z_e, emb)
                ...
                if args.use_vae:
                    ...
                    loss = loss_seq + loss_graph - args.kl_weight * \
                           (args.seq_graph_weight *  Graph_kl_loss)
                else:
            ...
            if (epoch + 1) % 1 == 0:
                with torch.no_grad():
                    ...
                    if valid_mse_saved > valid_mse:
                        ...
                        if args.use_vae:
                            torch.save(Seq_model.state_dict(), args.model_path + '{}_Cont_Seq_VQVAE.pt'.format(args.dataset))
                        else:
                            torch.save(Seq_model.state_dict(), args.model_path + '{}_Cont_Seq_VQVAE+Graph_VAE.pt'.format(args.dataset))
    ...
    # #---------------- Test Phase----------------#
    Seq_model_load =  VQ_VAE(args, k=args.k).to(device)
    Seq_model_load.load_state_dict(torch.load(args.model_path+'{}_Cont_Seq_VQVAE.pt'.format(args.dataset)))
    Seq_model_load.eval()

    with torch.no_grad():
        predicts = []
        for graphs,sequences,labels, pad_len in tqdm(test_loader):
            predict, hidden, z_e, emb = Seq_model_load(sequences)
            predicts = predicts + predict.cpu().detach().numpy().tolist()
        predicts = torch.tensor(predicts)
